...
class GesturePreview:
    ...
    def draw(self, template: np.ndarray = None):
        # Draw background
        sample_bg = pyglet.shapes.Rectangle(
            self.x, self.y, self.width, self.height, color=(220, 220, 220)
        )
        ...
        if n > 1:
            ...
            for i in range(n - 1):
                arrow_length += np.hypot(
                    t_disp[i + 1][0] - t_disp[i][0], t_disp[i + 1][1] - t_disp[i][1]
                )
            arrow_length *= 2
            ...
            if length > 0 and arrow_length > 0:
                ...
                pyglet.shapes.Line(
                    base_x, base_y, tip_x, tip_y, thickness=3, color=(255, 80, 0)
                ).draw()
                pyglet.shapes.Line(
                    tip_x, tip_y, left_x, left_y, thickness=3, color=(255, 80, 0)
                ).draw()
                pyglet.shapes.Line(
                    tip_x, tip_y, right_x, right_y, thickness=3, color=(255, 80, 0)
                ).draw()
        # Draw label
